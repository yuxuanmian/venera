...
class ComicSource {
  ...
  /// The managed context is present only for Catalog-owned sources. It keeps
  /// source data isolated while the source is being prepared and gates its
  /// callbacks after the Runtime is revoked.
  ...
  Future<void> loadData({ManagedSourceContext? context}) async {
    ...
    if (managedContext != null) {
      managedRuntimeBridge.require(managedContext);
      data = Map<String, dynamic>.from(managedContext.data);
      return;
    }
  ...
  Future<void> saveData({ManagedSourceContext? runtimeContext}) async {
  ...
  Map<String, Map<String, dynamic>>? getSettingsDynamic() {
    try {
      var value = JsEngine().runCode(
        "ComicSource.sources.$key.settings",
        null,
        runtimeContext,
      );
      if (value is Map) {
