...
class _CustomSliderState extends State<CustomSlider> {
  ...
  /// Computes the slider value at horizontal position [dx] and moves the
  /// thumb there.
  ///
  /// The change is committed to the parent (via [CustomSlider.onChanged])
  /// only when [notify] is true. A drag keeps [notify] false and commits once
  /// when the gesture ends, so a single jump never fires hundreds of
  /// [onChanged] callbacks per second — which previously restarted the
  /// reader's page animation, rebuilt the reader and decoded images on every
  /// pointer move, freezing the page.
  void _updateValue(double dx, double maxWidth, {required bool notify}) {
    if (maxWidth <= 0) {
      return;
    }
    if (widget.reversed) {
    ...
    var gap = maxWidth / widget.divisions;
    var gapValue = (widget.max - widget.min) / widget.divisions;
    var newValue = (dx / gap).round() * gapValue + widget.min;
